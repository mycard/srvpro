...
(function() {
  ...
  Room = class Room {
    ...
    delete() {
      var end_time, formatted_replays, index, len2, log_rep_id, m, name, player_ips, player_names, recorder_buffer, ref2, ref3, repbuf, replay_id, score, score_array, score_form;
      if (this.deleted) {
        return;
      }
  ...
  ygopro.ctos_follow('CHAT', true, function(buffer, info, client, server, datas) {
    ...
    switch (cmd[0]) {
      ...
      case '/tip':
        ...
        break;
      case '/ai':
        if (settings.modules.windbot.enabled && client.is_host && !room.arena && room.random_type !== 'M') {
          cmd.shift();
          if (name = cmd.join(' ')) {
            windbot = _.sample(_.filter(windbots, function(w) {
