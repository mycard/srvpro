...
(function() {
  ...
  CLIENT_get_authorize_key = global.CLIENT_get_authorize_key = function(client) {
    ...
    } else if (settings.modules.tournament_mode.enabled || client.is_local) {
      return client.name;
    } else {
    ...
    for (m = 0, len2 = ROOM_all.length; m < len2; m++) {
      ...
      if (room && room.duel_stage !== ygopro.constants.DUEL_STAGE.BEGIN && !room.windbot) {
        ...
        for (n = 0, len3 = ref2.length; n < len3; n++) {
          player = ref2[n];
          if (!player.closed && player.name === client.name && player.pass === client.pass && (settings.modules.tournament_mode.enabled || player.ip === client.ip || (client.vpass && client.vpass === player.vpass)) && (!deckbuf || _.isEqual(player.start_deckbuf, deckbuf))) {
            return player;
          }
        }
      }
  ...
  Room = class Room {
    ...
    delete() {
      var index, log_rep_id, name, player_ips, player_names, recorder_buffer, ref2, replay_id, score, score_array, score_form;
      if (this.deleted) {
        return;
      }
  ...
  ygopro.ctos_follow('JOIN_GAME', false, function(buffer, info, client, server, datas) {
    var len2, m, name, ref2, replay_id, room;
    //log.info info
    ...
    } else if (!settings.modules.tournament_mode.enabled && _.any(badwordR.level3, function(regexp) {
      return name.match(regexp);
    }, name = client.name)) {
      log.warn("BAD NAME LEVEL 3", client.name, client.ip);
      ygopro.stoc_die(client, "${bad_name_level3}");
    } else if (!settings.modules.tournament_mode.enabled && _.any(badwordR.level2, function(regexp) {
      return name.match(regexp);
    }, name = client.name)) {
      log.warn("BAD NAME LEVEL 2", client.name, client.ip);
      ygopro.stoc_die(client, "${bad_name_level2}");
    } else if (!settings.modules.tournament_mode.enabled && _.any(badwordR.level1, function(regexp) {
      return name.match(regexp);
    }, name = client.name)) {
  ...
  ygopro.ctos_follow('UPDATE_DECK', true, function(buffer, info, client, server, datas) {
    var buff_main, buff_side, card, current_deck, deck, deck_array, deck_main, deck_side, deck_text, deckbuf, decks, found_deck, i, len2, len3, line, m, n, room, struct;
  ...
  if (settings.modules.http) {
    ...
    requestListener = async function(request, response) {
      var archive_args, archive_name, archive_process, check, duellog, err, error, filename, getpath, len2, m, parseQueryString, pass_validated, ref2, replay, roomsjson, u;
